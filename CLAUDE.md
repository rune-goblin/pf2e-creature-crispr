# pf2e-creature-crispr — project rules
...
(`src/index.ts` → `dist/pf2e-creature-crispr.{js,css}`). `module.json` is the manifest.
...
- Module id `pf2e-creature-crispr`; flags, settings, the socket channel (`module.<id>`),
...
- Strings: `lang/en.json` under `pf2e-creature-crispr.*`; `game.i18n.localize/format`. No hard-coded strings.
...
- Release: tag `vX.Y.Z` → `release.yml` stamps the version, type-checks, builds, publishes `module.json` + `pf2e-creature-crispr.zip` (zip ships `dist lang packs assets` — must include the art).
