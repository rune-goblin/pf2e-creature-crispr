...
/**
 * Guarantee the world the harness launched has pf2e-creature-crispr enabled before any spec
 * runs. None of the dev worlds enable it by default (module activation is a per-world LevelDB
 * setting), so on first run we flip `core.moduleConfiguration` as GM and reload — this is what
 * Foundry's Module Management UI does, minus the manual click. The change persists, so later
 * runs find it already active. Fails loud rather than letting specs hit an undefined api.
 */
export default async function globalSetup(config: FullConfig): Promise<void> {
  ...
  try {
      ...
      if (!(await isActive())) {
        throw new Error(
          `Set ${MODULE_ID} active in core.moduleConfiguration but it's still inactive after reload. ` +
            `Open the test world once, enable "PF2E Creature CRISPR" in Manage Modules, return to setup, and re-run.`,
        );
      }
    }
    ...
    console.log(`[e2e] ${MODULE_ID} v${version} active in world "${env.world}" (pf2e) — public API ready.`);
  } finally {
    await browser.close();
  }
}
